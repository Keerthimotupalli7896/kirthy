...
@Component(service = customSchedular.class,immediate = true)
@Designate(ocd = configurationosgi.class)
public class customSchedular implements Runnable {

    private static final Logger log=LoggerFactory.getLogger(customSchedular.class);
    @Reference
    ...
    public void activate(configurationosgi config){
        log.info("Inside the activate modethod of customSchedular");
        this.Schedule(config);
    }
    @Deactivate
    public void deactivate(configurationosgi config){
        log.info("customSchedular job is done");
        scheduler.unschedule(config.getSchedularName());
    }
    @Override
    public void run() {
        log.info("customSchedular has been executed");
        }
}
